...
import org.realestate.db.entity.ContractRealestateType;
 ...
 * @author Pathompong
 */
@WebServlet(name = "setup.contract.ContractRealestateTypePage", urlPatterns = {"/setup/contract/contract_realestate_type"})
public class ContractRealestateTypePage extends HttpServlet implements PageServlet {
    ...
    protected void doGet(
        ...
        if (flag(request, "id")) {
            if (flag(request, Integer.class, "id")) {
                attr(request, "find", notnull(model(ContractRealestateType.class, Integer.class).find(param(request, Integer.class, "id")), "id = " + param(request, "id")));
            }
            jsp(request, response, ".input.jsp");
        } else {
            attr(request, "finds", model(ContractRealestateType.class).finds(blank(), order("id")));
    ...
    protected void doPost(
        ...
        ContractRealestateType entity;
        if (flag(request, Integer.class, "id")) {
            entity = notnull(model(ContractRealestateType.class, Integer.class).find(param(request, Integer.class, "id")), "id = " + param(request, "id"));
            if (flag(request, "del")) {
                entity.setUpdated(new Date());
                entity.setUpdater(access(request, setup_lookup_delete));
                if (model(ContractRealestateType.class, Integer.class).del(true, entity)) {
            ...
            entity = new ContractRealestateType();
        }
        entity.setUpdated(new Date());
        ...
        if (model(ContractRealestateType.class, Integer.class).put(entity)) {
            redirect(response, "contract_type");
        } else {
            throw ApplicationException.Type.uncommited_transaction.dispatch();
        }
