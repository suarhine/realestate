 ...
 * @author Pathompong
 */
@WebServlet(name = "setup.contract.ContractAppointmentTypePage", urlPatterns = {"/setup/contract/contract_appointment_type"})
public class ContractAppointmentTypePage extends HttpServlet implements PageServlet {
    ...
    protected void doGet(
        ...
        if (flag(request, "id")) {
            if (flag(request, Integer.class, "id")) {
                attr(request, "find", notnull(model(ContractAppointmentType.class, Integer.class).find(param(request, Integer.class, "id")), "id = " + param(request, "id")));
            }
            jsp(request, response, ".input.jsp");
        } else {
            attr(request, "finds", model(ContractAppointmentType.class).finds(blank(), order("id")));
    ...
    protected void doPost(
        ...
        ContractAppointmentType entity;
        if (flag(request, Integer.class, "id")) {
            entity = notnull(model(ContractAppointmentType.class, Integer.class).find(param(request, Integer.class, "id")), "id = " + param(request, "id"));
            if (flag(request, "del")) {
                entity.setUpdated(new Date());
                entity.setUpdater(access(request, setup_lookup_delete));
                if (model(ContractAppointmentType.class, Integer.class).del(true, entity)) {
            ...
            entity = new ContractAppointmentType();
        }
        entity.setUpdated(new Date());
        ...
        if (model(ContractAppointmentType.class, Integer.class).put(entity)) {
            redirect(response, "contract_type");
        } else {
            throw ApplicationException.Type.uncommited_transaction.dispatch();
        }
